...
namespace Inventra.Application.Features.Notifications.Handlers
{
    public sealed class GetUnreadNotificationCountQueryHandler: IRequestHandler<GetUnreadNotificationCountQuery,int>
    {
        ...
        public GetUnreadNotificationCountQueryHandler(INotificationReadRepository notificationReadRepository,ICurrentUserService currentUserService)
        {
        ...
        public async Task<int> Handle(GetUnreadNotificationCountQuery request,CancellationToken cancellationToken)
        {
            ...
            {
                var notifications =await _notificationReadRepository.GetWhereAsync(x => !x.IsRead,false,cancellationToken);

                return notifications.Count;
            }
            var unreadNotifications =await _notificationReadRepository.GetWhereAsync( x =>!x.IsRead&&((x.UserId != null &&x.UserId ==_currentUserService.UserId)||
                                (x.RoleName != null && _currentUserService.Roles.Contains(x.RoleName))),false,cancellationToken);
