...
namespace Inventra.Application.Features.Notifications.Handlers
{
    public sealed class MarkNotificationAsReadCommandHandler: IRequestHandler<MarkNotificationAsReadCommand,Result>
    {
        ...
        public MarkNotificationAsReadCommandHandler(INotificationReadRepository notificationReadRepository,IUnitOfWork unitOfWork)
        {
        ...
        {
            var notification =await _notificationReadRepository.GetByIdAsync(request.NotificationId,tracking: true,cancellationToken);
            ...
            return Result.SuccessResult("Bildirim okundu olarak işaretlendi.");
        }
    }
}